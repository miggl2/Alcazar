...
export type HoleMode = 'two-holes' | 'more-holes' | 'one-cycle';
...
export function buildInitialPuzzle(solution: PathState, rng: RNG): PuzzleGraph {
  ...
  const startDirs = boundaryDirs(start, graph.w, graph.h);
  const startDir = pick(rng, startDirs);
  openBoundaryHole(graph, start, startDir);

  if (start === end) {
    const remainingDirs = startDirs.filter((dir) => dir !== startDir);
    openBoundaryHole(graph, end, pick(rng, remainingDirs.length > 0 ? remainingDirs : startDirs));
  } else {
...
export function carvePuzzle(
  ...
  orderCandidatesForFastRejection(internalCandidates, solution, mode);
...
export function generatePuzzle(
  ...
  if (w < 1 || h < 1) {
    throw new Error('Alcazar puzzle width and height must be at least 1.');
  }
  ...
  startedAt = now();
  const carved = carvePuzzle(graph, solution, mode, carveRng, fullOptions, deadlineAt);
