...
export type HoleMode = 'two-holes' | 'multi-holes';
...
export function carvePuzzle(
  ...
  let startedAt = now();
  const candidates = makeInternalWallCandidates(graph);
  timings.internalCandidateMs += now() - startedAt;
  timings.internalCandidateCount = candidates.length;
  ...
  if (mode === 'multi-holes') {
    ...
    candidates.push(...holeCandidates);
  }
  timings.candidateCount = candidates.length;

  startedAt = now();
  shuffleInPlace(candidates, rng);
  orderCandidatesForFastRejection(candidates, solution, mode);
...
function buildForcedSearchGraph(graph: PuzzleGraph, candidate: Candidate): ForcedSearchGraph {
  const edgeU: number[] = [];
  const edgeV: number[] = [];
...
export function generatePuzzle(
  ...
  startedAt = now();
  const solution = generateSolutionPath(w, h, rng, fullOptions);
  const solutionMs = now() - startedAt;

  startedAt = now();
  const graph = buildInitialPuzzle(solution, rng);
